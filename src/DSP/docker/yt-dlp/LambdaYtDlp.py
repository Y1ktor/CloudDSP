...
def lambda_handler(event, context):
    ...
    try:
        ...
        # The postprocessor creates a .wav file
        final_file = f"/tmp/{temp_id}.wav"
        
        if not os.path.exists(final_file):
            error_payload = {'type': 'error', 'message': 'Failed to extract and convert audio to WAV'}
            send_ws_message(apigw_client, connection_id, error_payload)
            return {'statusCode': 500, 'body': json.dumps(error_payload)}
            
        # Define S3 Key
        s3_key = f"uploads/yt-dlp/{temp_id}-{title}.wav"
        ...
        # Upload the converted WAV file to S3 with connectionId and stemMode in metadata
        extra_args = {'Metadata': {}}
        if connection_id:
